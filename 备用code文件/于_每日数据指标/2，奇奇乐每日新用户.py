...
# 累加到累加数据 / 去重
df_lei = df_lei.append(df_form, ignore_index=True)
df_lei.drop_duplicates(keep='last', inplace=True)

# 导出到桌面
df_form.to_excel('C:\\Users\Administrator\Desktop\表格提取源\奇奇乐{}号新用户统计情况.xlsx'.format(ri_y2), index=False)
print('\n数据已导出到桌面……！')

# 数据存放累加数据
df_lei.to_excel('D:\MD_DATA\奇奇乐新用户叠加\\data.xlsx', index=False)
print('\n数据已累加到 data ……\n')
