...
def du_old_excel(excel_name,_type):
    try:
        if _type == '充值':
            # 读取数据
            df = pd.read_excel('C:\\Users\Administrator\Desktop\奇奇乐_新用户_留存\充值数据\\{}.xls'.format(excel_name))
            return df
        elif _type == '登入':
            # 读取数据
            df = pd.read_excel('C:\\Users\Administrator\Desktop\奇奇乐_新用户_留存\用户登入数据\\{}.xls'.format(excel_name))
            return df
    except FileNotFoundError:
        print('\n缺少运行数据，请先下载……')
        exit()
