...
def du_excel(excel_name):
    try:
        # 读取数据
        df = pd.read_excel('C:\\Users\Administrator\Desktop\\B_浪仔每日数据\\{}.xlsx'.format(excel_name))
        return df
...
def du_old_excel(excel_name):
    try:
        # 读取数据
        df = pd.read_excel('C:\\Users\Administrator\Desktop\\B_浪仔每日数据\\{}.xls'.format(excel_name))
        return df
    except FileNotFoundError:
        print('\n缺少运行数据，请先下载……')
        exit()
