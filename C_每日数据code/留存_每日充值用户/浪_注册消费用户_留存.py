...
import warnings
from build.database import date, url44, yesterday,url55,Register,url66,Login

warnings.filterwarnings('ignore')

from Func import append_excel

print(Register(date(url55)).shape[0])
exit()

Register(date(url55)).to_excel('C:\\Users\Administrator\Desktop\奇奇乐付费新用户留存1\注册数据\\{}.xlsx'.format(yesterday),index=False)

# Login(date(url66)).to_excel('C:\\Users\Administrator\Desktop\奇奇乐付费新用户留存1\登入数据\\{}.xlsx'.format(yesterday),index=False)
